...
def parse_gmu_catalog(file_path, major=None):
    ...
    # Extract the program ID from the URL or use a fallback
    program_id = None
    if major:
        program_id = major.lower().replace(' ', '_').replace(',', '').replace('/', '_')
    
    # Try to find the requirementstextcontainer
    requirements_container = soup.find('div', id='requirementstext')
    if not requirements_container:
        requirements_container = soup.find('div', id='requirementstextcontainer')
    
    if not requirements_container:
        # Fall back to legacy parsing when no requirements container is found
        return legacy_parse_gmu_catalog(soup, major, program_id)
    
    # Initialize results dictionary
    results = {
        "degree_name": major if major else "Unknown Program",
        "total_credits": 120,  # Default to 120, will be updated if found
        "categories": []
    }
    
    # Try to find the total credits required
    for heading in requirements_container.find_all(['h2', 'h3']):
        heading_text = heading.get_text(strip=True)
        if "degree requirements" in heading_text.lower() or "total credits" in heading_text.lower():
            credits_match = re.search(r'(\d+)\s*credits', heading_text, re.IGNORECASE)
            if credits_match:
                results["total_credits"] = int(credits_match.group(1))
                break
    
    # Find all category headers
    category_headers = requirements_container.find_all(['h3', 'h4', 'strong', 'b'])
    categories = []
    
    for header in category_headers:
        header_text = header.get_text(strip=True)
        if header_text and not header_text.isspace():
            # Standardize and process category name
            category_name = standardize_category_name(header_text)
            
            # Skip excluded categories
            if is_excluded_category(category_name):
                continue
            
            # Find the container for this category's courses
            category_element = header
            for elem in header.next_siblings:
                if elem.name in ['h3', 'h4', 'strong', 'b']:
                    break
                category_element = elem
            
            # Find all courses in this category element
            courses = find_courses_in_element(category_element)
            
            # Skip categories with no courses
            if not courses:
                continue
            
            # Find total credits required for this category
            total_credits = find_credits_required(category_element, category_name)
    ...
    # Extract possible concentrations
    concentrations = extract_concentrations(soup, program_id)
    if concentrations:
        results["concentrations"] = concentrations
    
    # Handle the special case for natural science requirements in Computer Science
    if major and "Computer Science" in major:
        ...
        for category in categories:
            ...
            # For categories with courses showing 0 credits, update them
            if category["total_credits"] > 0 and all(course["credits"] == 0 for course in category["courses"]):
                # Calculate average credit per course
        ...
        natural_science_cat = next((cat for cat in categories if cat["name"] == "Natural Science"), None)
        approved_sequences_cat = next((cat for cat in categories if "Approved Two-Course Sequence" in cat["name"]), None)
...
def save_results(results, major, output_dir=None):
    """
    Save the results to JSON and SQL files.
    ...
    # Clean the major name to use as a filename
    major_id = major.lower().replace(' ', '_').replace(',', '').replace('/', '_')
    
    # Save to JSON
    json_file = os.path.join(output_dir, f"{major_id}_requirements.json")
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2)
    print(f"Results saved to {json_file}")
    
    # Save to SQL
    sql_file = os.path.join(output_dir, f"{major_id}_requirements.sql")
    
    # Generate SQL insert statements
    sql_statements = []
    sql_statements.append(f"-- SQL insert statements for {major}")
    sql_statements.append(f"-- Generated on {datetime.datetime.now()}")
    sql_statements.append("")
    
    # Create a simple ID from the filename
    major_id = major_id.replace('_', '')[:30]
    
    # Insert into majors table
    sql_statements.append("-- Insert into majors table")
    sql_statements.append(f"INSERT INTO majors (id, name, total_credits) VALUES ('{major_id}', '{results['degree_name']}', {results['total_credits']});")
    sql_statements.append("")
    
    # Insert into categories table
    sql_statements.append("-- Insert into categories table")
    for i, category in enumerate(results['categories']):
        category_id = f"{major_id}_cat_{i}"
        sql_statements.append(f"INSERT INTO categories (id, major_id, name, credits_required) VALUES ('{category_id}', '{major_id}', '{category['name']}', {category['total_credits']});")
    
    sql_statements.append("")
    
    # Insert into courses table
    sql_statements.append("-- Insert into courses table")
    for i, category in enumerate(results['categories']):
        category_id = f"{major_id}_cat_{i}"
        for j, course in enumerate(category['courses']):
            ...
            sql_statements.append(f"INSERT INTO courses (id, category_id, course_code, title, credits) VALUES ('{course_id}', '{category_id}', '{course_code}', '{title}', {course['credits']});")
